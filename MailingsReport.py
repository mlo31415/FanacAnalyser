...
# mailings.csv that FanacAnalyzer writes; it now runs as FanacAnalyzer's last report, working directly from the
...
#   * the mailings FanacAnalyzer found on the fanzine index pages (which apazine was in which mailing)
#   * Joe's "APA Mailings.xlsx" (who the OE was and when each mailing came out), which FanacAnalyzer has no other use for
...
# fanacIssueList is the complete list of issues FanacAnalyzer has already read.
